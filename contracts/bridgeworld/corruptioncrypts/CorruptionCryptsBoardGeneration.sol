...
abstract contract CorruptionCryptsBoardGeneration is Initializable, CorruptionCryptsSettings {

    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;

    uint NUM_MAPTILES = 32;
    uint receiptId;

    ShittyRandom public shittyRandom = new ShittyRandom();

    function __CorruptionCryptsBoardGeneration_init() internal initializer {
        CorruptionCryptsSettings.__CorruptionCryptsSettings_init();
    }

    // function generateGameBoardForRequest(
    //     uint256 _requestId
    // ) external view returns(GridCell[3][3] memory) {
    //     // appStorage definedi n LibCorruptionCryptsDiamond.sol
    //     uint256 _randomNumber = appStorage.randomizer.revealRandomNumber(_requestId);

    //     return generateBoard(_randomNumber);
    // }

    function drawRandomMapTileId(uint _requestId) public returns (uint) {
        ...
        console.log("randInt:", _randomNumber);
        console.log("mapTileId:", mapTileId);
        return mapTileId;
    }

    // function generateBoard(
    //     uint256 _randomNumber
    // ) public view returns(GridCell[3][3] memory) {
    ...
    //         // uint256 _treasureId = appStorage.treasureMetadataStore.getAnyRandomTreasureForTier(_tier, _randomNumber);
